...
<template>
  <main class="mx-auto min-h-screen max-w-6xl px-4 py-8 sm:px-6 lg:py-12">
    <section
      class="rounded-3xl bg-gradient-to-br from-emerald-950 via-teal-900 to-slate-900 p-6 text-white sm:p-9"
    >
      <div class="flex flex-wrap items-start justify-between gap-5">
        <div>
          ...
          <p class="mt-3 max-w-2xl text-sm leading-relaxed text-emerald-50/85">
            เผยแพร่เรื่องราว สถานที่ หรือวิดีโอของคุณให้ทุกคนค้นพบได้ทันที
          </p>
        </div>
        <RouterLink
          to="/create"
          class="rounded-xl bg-white px-5 py-3 text-sm font-bold text-emerald-800 shadow-sm transition hover:bg-emerald-50"
        >
    ...
    <section class="mt-7">
      <div v-if="loading" class="grid gap-4 sm:grid-cols-2">
        <div v-for="index in 4" :key="index" class="h-44 animate-pulse rounded-2xl bg-slate-200" />
      </div>
      <div
      ...
      >
        ...
        <RouterLink
          to="/create"
          class="mt-6 inline-flex rounded-xl bg-emerald-600 px-5 py-3 font-bold text-white hover:bg-emerald-700"
          >สร้างคอนเทนต์</RouterLink
        >
      </div>
        ...
        >
          <div class="flex items-start justify-between gap-3">
            <span
              ...
              >{{ statusLabel(content.status) }}</span
            ><span class="text-xs text-slate-400">{{ formatDate(content.updatedAt) }}</span>
          </div>
          ...
          <p class="mt-2 line-clamp-2 min-h-10 text-sm text-slate-500">
            {{ content.summary || 'ไม่มีคำอธิบายย่อ' }}
          </p>
          <div class="mt-4 flex items-center justify-between border-t border-slate-100 pt-4">
            <RouterLink
              v-if="content.status !== 'Archived'"
              :to="`/my-contents/${content.contentId}/edit`"
