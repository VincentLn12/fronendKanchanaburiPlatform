...
export async function getPublicContents(params: { search?: string; categoryId?: string; districtId?: string; subDistrictId?: string; tagId?: string; shopId?: string; page?: number; pageSize?: number }) { const { data } = await http.get<PagedResult<PublicContent>>('/contents', { params }); return data }
