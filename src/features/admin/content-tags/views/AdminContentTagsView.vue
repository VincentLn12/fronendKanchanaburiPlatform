...
import { getContentTags, getContents, getTags, replaceContentTags, type ContentOption, type TagOption } from '../api/adminContentTagApi'
...
async function loadTagAssignments() {
  if (!selectedContentId.value) { selectedTagIds.value = []; return }
  ...
  try {
    ...
    await swal.error('โหลดแท็กของคอนเทนต์ไม่สำเร็จ', getApiErrorMessage(error, 'กรุณาลองใหม่อีกครั้ง'))
...
<template>
  <main class="mx-auto w-full max-w-3xl px-6 py-10">
    ...
    <section v-else class="space-y-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <AppSelect v-model="selectedContentId" label="เลือกคอนเทนต์" :items="contents" item-title="title" item-value="contentId" placeholder="เลือกคอนเทนต์ที่ต้องการจัดการแท็ก" clearable />
      <template v-if="selectedContentId">
        <div v-if="loadingTags" class="h-1 animate-pulse rounded bg-indigo-600" />
        <template v-else>
          <AppSelect v-model="selectedTagIds" label="แท็ก" :items="tags" item-title="tagName" item-value="tagId" multiple class="min-h-40" />
          <p class="text-xs text-slate-500">กด Ctrl (Windows) หรือ Command (Mac) เพื่อเลือกมากกว่าหนึ่งแท็ก</p>
          <div class="flex flex-wrap gap-2">
            <span v-for="tagId in selectedTagIds" :key="tagId" class="rounded-full bg-indigo-50 px-3 py-1 text-xs font-semibold text-indigo-700">#{{ tags.find((tag) => tag.tagId === tagId)?.tagName }}</span>
            <span v-if="!selectedTagIds.length" class="text-sm text-slate-400">ยังไม่ได้เลือกแท็ก</span>
          </div>
          <button class="w-full rounded-xl bg-indigo-600 px-5 py-3 font-semibold text-white hover:bg-indigo-700 disabled:opacity-50" :disabled="saving" @click="save">{{ saving ? 'กำลังบันทึก...' : 'บันทึกแท็ก' }}</button>
        </template>
      </template>
      <p v-else class="rounded-xl bg-slate-50 p-5 text-center text-sm text-slate-500">เลือกคอนเทนต์เพื่อเริ่มจัดการแท็ก</p>
    </section>
  </main>
</template>
